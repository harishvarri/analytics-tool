...
/**
 * Departments — the "Departments" tab of Productivity Insights. Consolidates the
 * former Department Intelligence page (headcount + activity) and the engagement
 * page's week-over-week department trend into one place (P2 consolidation).
 */
export async function DepartmentsPanel() {
  const [activity, rollup, deptWeek, deptTwoWeek] = await Promise.all([
  ...
  const mostActive = activity[0] ?? null;
  ...
  // Week-over-week trend (this 7d vs prior 7d, derived from 14d − 7d).
  const prevByDept = new Map(deptTwoWeek.map((d) => [d.department, d.events]));
  const deptTrend = deptWeek
    .map((d) => {
      const prev = Math.max(0, (prevByDept.get(d.department) ?? 0) - d.events);
    ...
    <div className="space-y-6">
      ...
      <section className="grid gap-4 xl:grid-cols-2">
                  ...
                  {activity.map((d) => (
                    <tr key={d.department} className="border-b last:border-b-0 hover:bg-muted/40">
                      <td className="px-2 py-2 font-medium">
                        <Link href={`/dashboard/departments/${encodeURIComponent(d.department)}`} className="hover:underline">{d.department}</Link>
                      </td>
        ...
        <ChartCard title="Department engagement" description="This week vs the previous week, by department">
          {deptTrend.length === 0 ? (
            <div className="flex h-[140px] items-center justify-center text-xs text-muted-foreground">No department activity yet.</div>
          ) : (
      ...
      {hasHeadcount && (
        <ChartCard title="Headcount by department" description="Directory composition — active, inactive, and invited staff per department">
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              <thead className="text-muted-foreground">
                <tr className="border-b">
