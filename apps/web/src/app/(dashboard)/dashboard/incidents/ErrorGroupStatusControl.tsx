...
/**
 * Resolve/close a single error signature. Optimistic, mirrors
 * IncidentStatusControl. Resolving/closing stops this signature's errors from
 * reducing the affected products' health.
 */
export function ErrorGroupStatusControl({ fingerprint, current }: { fingerprint: string; current: IncidentStatus }) {
  ...
  const choose = (next: IncidentStatus) => {
    ...
    startTransition(async () => {
      const res = await updateErrorGroupStatus(fingerprint, next);
