...
export default async function EngagementIntelligencePage() {
  const [active, people, projects, journeys] = await Promise.all([
    fetchActiveUserCounts(),
  ...
  const mostActive = [...withActivity].sort((a, b) => b.totalEvents - a.totalEvents).slice(0, 6);
  const leastActive = [...withActivity].sort((a, b) => a.totalEvents - b.totalEvents).slice(0, 6);
  ...
  const growing = projects.filter((p) => p.activityNorm > 55).sort((a, b) => b.activityNorm - a.activityNorm).slice(0, 4);
  const declining = projects.filter((p) => p.activityNorm < 45).sort((a, b) => a.activityNorm - b.activityNorm).slice(0, 4);
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="Who is engaged, who has gone quiet, and which products are growing or declining."
      ...
      {/* Primary row: most active · least active · product engagement (3-up) */}
      <section className="grid items-start gap-6 lg:grid-cols-3">
        <ChartCard title="Most active staff" description="Getting the most done across products"
        ...
        <ChartCard title="Least active staff" description="Engaged lightly — enablement candidates">
          <Leaderboard rows={leastActive} />
        </ChartCard>
        <ChartCard title="Product engagement" description="Used, growing & declining products">
          ...
          <div className="mt-3 grid gap-3 sm:grid-cols-2">
            <div>
              ...
              {growing.length === 0 ? <div className="text-xs text-muted-foreground">None</div> : (
                <ul className="space-y-1 text-xs">{growing.map((p) => <li key={p.slug} className="flex justify-between gap-2"><span className="truncate">{p.name}</span><span className="tabular-nums text-muted-foreground">{p.activityNorm}</span></li>)}</ul>
              )}
            </div>
            <div>
              ...
              {declining.length === 0 ? <div className="text-xs text-muted-foreground">None</div> : (
                <ul className="space-y-1 text-xs">{declining.map((p) => <li key={p.slug} className="flex justify-between gap-2"><span className="truncate">{p.name}</span><span className="tabular-nums text-muted-foreground">{p.activityNorm}</span></li>)}</ul>
              )}
            </div>
          </div>
        </ChartCard>
      ...
      {/* Secondary row: gone quiet (table) · common workflows (2-up) */}
      <section className="grid items-start gap-6 lg:grid-cols-2">
        <ChartCard title="Staff who went quiet" description="Recently active people who haven't returned"
        ...
        <ChartCard title="Common operational workflows" description="Actions staff most often perform back-to-back (page views & clicks excluded)">
...
function Leaderboard({ rows }: { rows: Array<{ userId: string; displayName: string | null; email: string | null; totalEvents: number; totalSessions: number }> }) {
