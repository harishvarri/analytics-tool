...
import { DepartmentsPanel } from '@/features/productivity/DepartmentsPanel';
...
const TABS = ['people', 'departments'] as const;
type TabKey = (typeof TABS)[number];

export default async function ProductivityInsightsPage({ searchParams }: PageProps) {
  const { tab } = await searchParams;
  const active: TabKey = (TABS as readonly string[]).includes(tab ?? '') ? (tab as TabKey) : 'people';

  const [counts, people, projects, journeys] = await Promise.all([
    fetchActiveUserCounts(),
        ...
        <TabsContent value="people">
          <div className="space-y-6">
            <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
              <KpiCard label="Active today" value={fmt.format(counts.dau)} icon={UserPlus} trend={{ direction: 'flat', label: 'last 24h' }} />
              <KpiCard label="Active this week" value={fmt.format(counts.wau)} icon={Users} trend={{ direction: 'flat', label: 'last 7d' }} href="/dashboard/sessions" />
              <KpiCard label="Active this month" value={fmt.format(counts.mau)} icon={Users} trend={{ direction: 'flat', label: 'last 30d' }} />
